...
export function GatewayDetailTimelineRow({
    ...
    const durationS = timeInStatusesS.active + timeInStatusesS.inactive + timeInStatusesS.unstable + timeInStatusesS.offline;
    ...
    return (
        <tr>
            ...
            <td className="gateway-stats-timeline-table-body-active" style={{ opacity: shareActive ? 1 : 0 }}>
                {shareActive > 0 ? <span>{numberFormatter.format(shareActive)}</span> : null}
            </td>
            <td className="gateway-stats-timeline-table-body-inactive" style={{ opacity: shareInactive ? 1 : 0 }}>
                {shareInactive > 0 ? <span>{numberFormatter.format(shareInactive)}</span> : null}
            </td>
            <td className="gateway-stats-timeline-table-body-unstable" style={{ opacity: shareUnstable ? 1 : 0 }}>
                {shareUnstable > 0 ? <span>{numberFormatter.format(shareUnstable)}</span> : null}
            </td>
            <td className="gateway-stats-timeline-table-body-offline" style={{ opacity: shareOffline ? 1 : 0 }}>
                {shareOffline > 0 ? <span>{numberFormatter.format(shareOffline)}</span> : null}
            </td>
