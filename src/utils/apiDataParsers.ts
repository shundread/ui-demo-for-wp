import {
  GatewayListingResponse,
  SingleGatewayStatsResponse,
} from "../api/GatewayApiTypes";
...
// TODO use zod
export function parseGatewayListingResponse(
  data: GatewayListingResponse,
): GatewayListItemProps[] {
  return data.results.map((apiGateway) => ({
    description: apiGateway.description,
    gatewayId: apiGateway.gatewayId,
    latestMessageTime: parseApiTimestamp(
      apiGateway.gatewayStatistics.lastMessageRxTime,
    ),
...
export function parseGatewayStatsResponse(
  data: SingleGatewayStatsResponse,
): Pick<GatewayDetailProps, "summary" | "timeline"> {
  return {
    summary: {
