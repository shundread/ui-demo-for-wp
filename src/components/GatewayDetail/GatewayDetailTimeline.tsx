...
import {
  GatewayStatsHistorySample,
  GatewayStatsStatusChangeEvent,
} from "./GatewayDetailTypes";
...
export function GatewayDetailTimeline({
  historySamples,
  statusChangeEvents,
}: GatewayDetailTimelineProps) {
  ...
  return (
    <section>
      ...
      <p>
        <em>
          <b>Note:</b> the timeline may appear distorted at the very first row
          or very last row of the interval due to it potentially having less
          data than the bin size at the extremities
        </em>
      </p>
        ...
        <table className="gateway-stats-timeline-table">
          ...
          <tbody className="gateway-stats-timeline-table-body">
            {historySamples.map((sample, index) => (
              <GatewayDetailTimelineRow
                {...sample}
                index={index}
                key={sample.startTime.valueOf()}
